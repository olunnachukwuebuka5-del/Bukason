...
// Buttons stay disabled until the Supabase client has actually loaded —
// tapping before then is what caused "Cannot read properties of null".
loginBtn.disabled = true;
signupBtn.disabled = true;
showAuthNotice('Connecting…');
...
function sniffMimeFromBytes(bytes){
  ...
  // HEIC/HEIF: bytes 4-7 spell "ftyp", followed by a brand code at 8-11.
  // Common camera-photo brands: heic/heix/hevc/hevx (HEIC), mif1/msf1 (HEIF).
...
function showAttachError(msg){
  ...
  setTimeout(() => { attachErrorEl.style.display = 'none'; }, 9000);
}
...
async function handleFilesSelected(files){
  ...
  for(const file of Array.from(files).slice(0, room)){
    ...
    try{
      ...
      // the fastest way to tell "bucket missing" from "RLS blocked" from
      // "network error" apart when something goes wrong.
      console.error('Attachment upload error:', err);
      const reason = (err && err.message) ? err.message : 'unknown error';
      showAttachError((localAtt.name || 'File') + ': ' + reason);
      removeAttachment(id);
    }
  }
}
...
async function sendMessage(){
  ...
  // ---- OFFLINE: no attachments supported offline (vision needs the cloud) ----
  if(!navigator.onLine){
    if(readyAtts.length){
      addMessage('bot', "File and image understanding needs an internet connection — I'll be able to look at this once you're back online.");
      sendBtn.disabled = false; attachBtn.disabled = false;
      return;
    }

    const offlineAI = window.BukasonOfflineAI;
    ...
    if(offlineAI && offlineAI.isReady()){
      ...
      try{
        ...
        // Supabase needs a connection — queue the save instead of calling it directly.
      ...
      addMessage('bot', offlineAI && offlineAI.hasFailed()
        ? "Offline AI isn't available on this device — this message will be answered once you're back online."
        : "Offline AI is still getting ready — this message will be answered once you're back online.");
    }
    sendBtn.disabled = false;
    attachBtn.disabled = false;
    return;
...
// ---------------- Offline queue sync ----------------
// When the connection comes back, resend anything that was only answered
// offline (or queued unanswered) so it lands in permanent Supabase history.
window.addEventListener('online', async () => {
  const offlineAI = window.BukasonOfflineAI;
  if(!offlineAI || !currentUser) return;
  const queued = offlineAI.drainQueue();
  for(const item of queued){
    try{
      ...
      console.error('Failed to sync a queued offline message:', err);
    }
  }
});
